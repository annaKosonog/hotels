...
public class HotelDtoPostAndPutControllerTest extends SampleHotel {
    ...
    void should_return_status_200_when_correctly_update_hotel(@Autowired MockMvc mockMvc, @Autowired ObjectMapper objectMapper) throws Exception {
        ...
        final MvcResult result = mockMvc.perform(put("/api/v1/dto/hotels/{partnerCode}", partnerCode)
                .contentType(MediaType.APPLICATION_JSON)
                .content(expected))
    ...
    @Test
    void should_return_status_204_on_invalid_update_hotel(@Autowired MockMvc mockMvc, @Autowired ObjectMapper objectMapper) throws Exception{
        final String partnerCode = "0b25d2b9-5573-4d38-a81f-de982bb554b6";

        final MvcResult result = mockMvc.perform(put("/api/v1/dto/hotels/{partnerCode}", partnerCode)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(invalidUpdateClarkHotelDto())))
                .andExpect(status().isBadRequest())
                .andDo(print())
                .andReturn();
